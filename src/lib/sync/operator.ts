import { sendFileInventory } from "./file_protocol";
import { sendNoteInventory } from "./note_protocol";
import { sendSyncInBatches, settleBatchSendSessionOnSyncEnd } from "./batch_sync";
export { settleAllBatchSendSessionsOnClose } from "./batch_sync";
...
async function receiveSyncEndWrapper(data: unknown, plugin: FastSync, type: "note" | "file" | "config" | "folder") {
  ...
  settleBatchSendSessionOnSyncEnd(plugin.websocket, batchAckEventByType[type]);
...
export const handleRequestSend = async function (plugin: FastSync, syncMode: SyncMode, noteData: NoteSyncData, fileData: FileSyncData, configData: ConfigSyncData, folderData: FolderSyncData) {
  ...
  if (plugin.settings.syncEnabled && shouldSyncNotes) {
    ...
    jobs.push(sendSyncInBatches(
      { websocket: plugin.websocket, syncState: plugin.syncState, debug: dump },
      "FolderSync",
      "FolderSyncBatchAck",
      folderData.context,
      folderData.folders,
    ...
    jobs.push(sendNoteInventory(
      { websocket: plugin.websocket, syncState: plugin.syncState, debug: dump },
      plugin.settings.vault, plugin.settings.offlineDeleteSyncEnabled, noteData,
      () => {
    ...
    if (!plugin.settings.cloudPreviewEnabled || plugin.settings.cloudPreviewTypeRestricted) {
      jobs.push(sendFileInventory(
        { websocket: plugin.websocket, syncState: plugin.syncState, debug: dump },
        plugin.settings.vault, plugin.settings.offlineDeleteSyncEnabled, fileData
      ));
    }
  }
